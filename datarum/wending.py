...
class wending(object):
    ...
    def __new__(self, gere, mónþ, dæg, tid=0, minute=0, second=0, microsecond=0):
        self = object.__new__(self)

        if not self.MINYEAR <= gere <= self.MAXYEAR:
            raise ValueError(
                "Gere must be in range {0}..{1}".format(self.MINYEAR, self.MAXYEAR),
                gere,
            )
        if not 1 <= mónþ <= 13:
        ...
        if mónþ == 13:
            ...
            if not 1 <= dæg <= max_days:
                raise ValueError(
                    "Dæg must be in range 1..{} for a Wending day".format(max_days), dæg
                )
        if not 0 <= tid <= 23:
    ...
    def strptime(cls, wending_string, format_string):
        ...
        if daeg == None:
            raise ValueError("No valid dæg found in wending string.")
        if month == None:
            raise ValueError("No valid mónþ found in wending string.")
        if gere == None:
            raise ValueError("No valid gere found in wending string.")
    ...
    def __str__(self):
        return "{0}.{1}.{2} {3}".format(self._gere, self._mónþ, self._dæg, self.time())
    ...
    def __add__(self, other):
        ...
        if delta.days > 0:
            return to_wending_from_ordinal(delta.days).replace(
